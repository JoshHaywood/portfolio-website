      ...
      <div class="h-full flex flex-col md:flex-row md:w-full mt-12 space-y-5 md:space-y-0 md:space-x-5 lg:space-x-10">
        {cards.map((card, index) => (
          ...
          >
            ...
            <a href="https://github.com/JoshHaywood?tab=repositories" class="w-full">
              <h3 class="text-xl font-medium">{card.heading}</h3>
              <p class="mt-4 text-gray-400 hover:underline">{card.text}</p>
            </a>

            <a href="https://github.com/JoshHaywood?tab=repositories" class="text-sm text-gray-600 absolute bottom-8 hover:underline">{card.projectNumber}+ projects</a>
          </div>
        ))}
      </div>
    </motion.div>
