...
export default function Projects() {
    ...
    return (
        <div className="md:w-3/5 mx-auto pt-8 md:pt-16 xl:pt-36 pb-12 md:pb-20 xl:pb-44"> 
            {/* Heading */}
                            ...
                            <div id={items.id} className="xl:pt-12 xl:space-x-10 xl:flex items-center text-right mt-20"> {/* Objects mapped ID used so all mapped elements have scroll listener applied */}
                                ...
                                <div className="relative w-3/5  overflow-hidden"> {/* Container for overlaying child elements */}
                                    <div className="hover:animate-pop-out over">  {/* Animation added as child so parent elements scale isnt effected */}
                                        <div className="absolute bg-primary w-full h-full z-10 opacity-30 hover:opacity-0 transition duration-300 ease-in-out"></div>
                                        <img className="rounded-sm h-full w-min " alt="Image of feature project" src={items.imageSrc}></img>
                                    </div>
                                </div>

                                {/* Text card */}
                                <div className="flex flex-col items-end z-20 w-2/5">
                                    <h6 className="text-primary">{items.tagline}</h6>
                                    <h4 className="mb-5">{items.heading}</h4>

                                    <p className="bg-senary pl-10 pr-5 pt-5 pb-5 -ml-32 shadow-md hover:shadow-lg rounded-sm text-gray-300">
                                        {items.text}
                                    </p>
                ...
                {items.map((items, index) => 
                    index == 1 && (
                        <Link to={items.link}>
                            <div id={items.id} className="xl:pt-12 xl:space-x-10 xl:flex items-center text-right mt-20"> {/* Objects mapped ID used so all mapped elements have scroll listener applied */}
                                {/* Text card */}
                                <div className="flex flex-col items-start z-20 w-2/5">
                                    <h6 className="text-primary">{items.tagline}</h6>
                                    <h4 className="mb-5">{items.heading}</h4>

                                    <p className="bg-senary text-left pl-5 pr-10 pt-5 pb-5 -mr-32 shadow-md hover:shadow-lg rounded-sm text-gray-300">
                                        {items.text}
                                    </p>
