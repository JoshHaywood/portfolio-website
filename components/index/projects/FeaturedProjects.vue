<template>
  <div
    ...
    "
    class="md:bg-none bg-center max-md:shadow-[inset_0_2000px_0_0_rgba(76,107,193,0.3)] md:flex cursor-pointer"
  >
    <!-- Image -->
    <div
      :class="card.alignment === 'right' ? 'order-2' : 'order-1'"
    ...
    >
      <div class="hover:scale-105 transition duration-300 ease-in-out">
        <NuxtImg :src="`/images/${card.image}`" :alt="`${card.name} thumbnail`" class="min-w-[650px] h-auto" />
      </div>
    </div>

    <!--Project information -->
    <div
      :class="card.alignment === 'right' ? 'md:text-right' : 'md:text-left'"
      class="md:w-2/5 md:z-10 p-5 sm:px-5 md:px-0 sm:py-16 md:py-10 max-md:backdrop-brightness-[0.3]"
    >
      <div class="font-normal text-primary">{{ card.tagline }}</div>
      <div class="mb-5 text-3xl font-semibold text-white hover:text-primary transition duration-300 ease-in-out">
        {{ card.heading }}
      </div>
      <p
        :class="card.alignment === 'right' ? 'md:pl-10 md:pr-5 md:-ml-32' : 'md:pl-5 md:pr-10 md:-mr-32'"
      ...
      <!-- Technologies -->
      <ul
        :class="card.alignment === 'right' ? 'md:justify-end' : 'md:justify-none'"
        class="flex flex-row flex-wrap gap-x-2.5 sm:gap-x-0 sm:space-x-5 md:mt-5 text-gray-500"
      >
        <li v-for="technology in card.technologies" :key="technology">{{ technology }}</li>
      </ul>
